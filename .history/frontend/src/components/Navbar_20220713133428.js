...
function Navbar() {
  return (
    <header className="flex space-x-4 items-center justify-between align-middle  p-4 bg-blue-400">
      ...
      <div class="flex items-center justify-center">
        <div class="xl:w-96">
          <div class="input-group flex  items-stretch w-full mb-4 rounded">
            <input
              type="search"
              class="form-control  flex-auto min-w-0 block w-full px-3 py-1.5 text-base font-normal   bg-clip-padding border border-solid border-blue-300 rounded transition ease-in-out m-0 focus:text-gray-800 focus:bg-white focus:border-blue-600 focus:outline-none opacity-25"
              placeholder="Search"
              aria-label="Search"
      ...
      <div >
        <ul className='hidden md:flex space-x-3 text-center'>
          <li>csdc</li>
          <li>csdc</li>
          <li>csdc</li>
        </ul>
      </div>
      <div class="space-x-4 text-gray-700 flex  px-3   text-center whitespace-nowrap rounded">
        <BsCart2 />
        <BsPersonCircle />
      </div>
    </header>
